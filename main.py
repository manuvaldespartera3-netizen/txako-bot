...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    canal_tutoria = config.CANALES.get('tutoria', 0)
    es_chat_maestro = update.effective_chat.id == config.MY_CHAT_ID
    es_canal_tutoria = canal_tutoria and update.effective_chat.id == canal_tutoria

    if not es_chat_maestro and not es_canal_tutoria:
        return

    # Si el mensaje viene del canal de tutoría — solo procesar confirmaciones
    if es_canal_tutoria:
        ...
        if hay_pendiente:
            response = await tutoria.handle(update.message.text, chat_id)
            await send_to_channel(context.bot, 'tutoria', response, update.effective_chat.id)
        return

    # Desde aquí — chat maestro
